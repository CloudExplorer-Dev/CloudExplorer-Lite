...
public class MappingUtil {
    ...
    public static F2CHost toF2CHost(Host host, List<Qemu> qemuList) {
        ...
        long cpuTotal = (long) CommonUtil.orDefault(host.getNode().getMaxcpu(), 0) * cpuMHzPerOneCore.getKey();
        long cpuUsed = CommonUtil.orDefault(host.getNode().getCpu(), BigDecimal.ZERO).multiply(BigDecimal.valueOf(cpuTotal)).longValue();
        f2CHost.setCpuMHzTotal(cpuTotal);
        f2CHost.setCpuMHzUsed(cpuUsed);
        f2CHost.setCpuMHzAllocated(cpuTotal - cpuUsed);
        long memTotal = CommonUtil.orDefault(host.getNode().getMaxmem(), 0L) / 1024 / 1024;
        long memUsed = CommonUtil.orDefault(host.getNode().getMem(), 0L) / 1024 / 1024;
        ...
        f2CHost.setNumCpuCores(CommonUtil.orDefault(host.getNode().getMaxcpu(), 0));
