...
public class BillingPolicyController {
    ...
    @GetMapping()
    @ApiOperation(value = "查询所有计费策略 ", notes = "查询所有计费策略")
    ...
    @ApiOperation(value = "查询当前策略可关联的云账号")
    @PreAuthorize("hasAnyCePermission('BILLING_POLICY:READ')")
    ...
    @ApiOperation(value = "删除一个计费策略")
    @PreAuthorize("hasAnyCePermission('BILLING_POLICY:DELETE')")
    public ResultHolder<Boolean> removeBillingPolicy(@ApiParam("计费策略Id")
    ...
    public ResultHolder<BillPolicy> updateBillingPolicy(@ApiParam("计费策略id") @CustomValidated(mapper = BaseBillPolicyMapper.class, handler = ExistHandler.class, field = "id", message = "计费策略id不存在", exist = false)
                                                        @PathVariable("billing_policy_id") String billingPolicyId,
